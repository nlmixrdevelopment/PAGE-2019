...
#----------------------------------------------------------
# Theophylline model using linCmt and with WT as a covaraie on CL
m3 <- function() {
  ini({
    tka <- .45
    tcl <- 1
  ...
  model({
    ka <- exp(tka + eta.ka)
    cl <- exp(tcl + beta.wt * WT + eta.cl)
    v <- exp(tv + eta.v)
    linCmt() ~ add(add.sd)
  })
}

fit3 <- nlmixr(m3, theo_sd, est="saem", table=tableControl(cwres=TRUE, npde=TRUE))

print(fit3)

#fit3 <- fit3 %>% addCwres() # In case this was not specified under model fit prior to estimation one could add this here to the results


#GoF by xpose
xpdb <- xpose_data_nlmixr(fit3)
