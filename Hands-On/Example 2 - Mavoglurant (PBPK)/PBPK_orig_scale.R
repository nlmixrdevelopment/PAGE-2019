...
dat$EVID = 1501*dat$EVID
dat$ID = cumsum(c(T,diff(dat$ID)!=0))
dat$occ = unlist(with(dat, tapply(EVID, ID, function(x) cumsum(x>0))))
dat = subset(dat, occ==1)
dat = subset(dat, ID<20)
dat = subset(dat, EVID>0 | DV>0)

fit <- nlmixr(pbpk, dat, est="saem", saemControl(n.burn=100, n.em=100))

xpdb <- xpose_data_nlmixr(fit)

##Goodness-of-fit plots
pdf(file="GOF plots case example 2 PBPK_original scale.pdf", height=9, width=9, paper="letter")
dv_vs_pred(xpdb) + ylab("Observed Mavoglurant Concentrations (ng/mL)") + xlab("Population Predicted Mavoglurant Concentrations (ng/mL)") 
dv_vs_ipred(xpdb) + ylab("Observed Mavoglurant Concentrations (ng/mL)") + xlab("Individual Predicted Mavoglurant Concentrations (ng/mL)")
res_vs_pred(xpdb) + ylab("Conditional Weighted Residuals") + xlab("Population Predicted Mavoglurant Concentrations (ng/mL)")
res_vs_idv(xpdb) + ylab("Conditional Weighted Residuals") + xlab("Time (h)")
prm_vs_iteration(xpdb) 
absval_res_vs_idv(xpdb, res = 'IWRES') + ylab("Individual Weighted Residuals") + xlab("Time (h)")
absval_res_vs_pred(xpdb, res = 'IWRES') + ylab("Individual Weighted Residuals") + xlab("Population Predicted Mavoglurant Concentrations (ng/mL)")
ind_plots(xpdb, nrow=3, ncol=4) + ylab("Predicted and Observed Mavoglurant Concentrations (ng/mL)") + xlab("Time (h)")
